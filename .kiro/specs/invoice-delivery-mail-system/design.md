...
- **プレビューPDF**: 処理予定一覧のプレビューボタン押下時にオンデマンド生成。S3には保存しない。タイトルに「プレビュー」文言、ファイル名に「_preview」を含める
...
class PdfGenerationService:
    async def generate_pdf(
        ...
        - preview=True（プレビュー）: S3に保存しない、pdf_bytesを返す、タイトルに「プレビュー」文言、ファイル名に「_preview」を含める（例: INV-2026-0401-001_preview.pdf）
...
class DocumentStorage:
    ...
    async def move_to_processed(
        ...
        手順（バッチ処理から呼ばれる前にmark_as_processedが実行済みであること）:
...
**確認待ちセクション** (2.1-2.11):
- 請求書・納品書をセクション分けして縦に並列表示
- dash_table.DataTableによる一覧表示（伝票番号、顧客名、営業担当、送信ON/OFF、合計金額、明細数）
- 営業担当フィルター
- 送信ON/OFFトグル（備考2のデフォルト値が初期値、変更はlocalStorageに保存）
- データ再取得時はlocalStorageの設定をクリアし備考2のデフォルト値に戻す
- 詳細ボタン -> モーダルで明細一覧表示（読み取り専用、編集不可）
- PDFプレビューボタン（確定前に送信内容を確認可能。オンデマンドでPDF生成し別タブ表示、S3には保存しない。タイトルに「プレビュー」文言を含める）
...
- PDFプレビューボタン（押下時にオンデマンドでPDF生成し別タブ表示、S3には保存しない。タイトルに「プレビュー」文言、ファイル名に「_preview」を含める）
- 複数書類選択と確定解除ボタン
- 確定解除時、対象が既にバッチ送信済みで存在しない場合は「既に送信処理されています。画面を更新してください」と表示
- 「更新」ボタンで最新の処理予定状態を取得
- 処理予定件数のステータスカウント
